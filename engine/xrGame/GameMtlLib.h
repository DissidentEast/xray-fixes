...
public:
	...
	IC u16				GetMaterialIdx	(LPCSTR name)	{GameMtlIt it=GetMaterialIt(name);if (materials.end()==it) Msg("! [MTL] material [%s] not found (%d loaded)", name, (int)materials.size());VERIFY(materials.end() != it); return (u16)(it-materials.begin());}
	IC SGameMtl*	IC SGameMtl*		GetMaterialByIdx(u16 idx)		{
